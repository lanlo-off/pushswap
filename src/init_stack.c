...
/*   Updated: 2025/05/31 16:09:22 by llechert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	indexation(t_stack **stack, int stack_size)
{
	...
	while (--stack_size > 0)//l'indexation se passe du plus grand au plus petit donc on repete stack_size fois l'operation
	{
		...
		while (tmp)
		{
			if (tmp->value == INT_MIN && tmp->index == 0)//si on rencontre INT MIN et qu'on l'a pas encore indexe on le met forcement a l'index 1
				tmp->index = 1;
			if (tmp->value > value && tmp->index == 0)//c'est ici qu'on exclut les elems deja indexes
			{
				value = tmp->value;
				highest = tmp;//on marque cet element comme etant le plus grand rencontre jusqu'a maintenant
				tmp = *stack;
			}
			else
		...
		if (highest != NULL)//lorsqu'on a parcouru toute la liste on connait le plus grand chiffre
			highest->index = stack_size;//on lui assigne donc l'index max de ce moment et on recommence pour l'index inferieur
	}
}
